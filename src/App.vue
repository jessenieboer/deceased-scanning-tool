...
export default {
  ...
  methods: {
    age(record) {
      if (record['Birthday'])
        return 2021 - record['Birthday'].split('/')[2];
      else if (record['Birthdate'])
    ...
    findAGraveUrl(item) {
      ...
      (item.Birthday || item.Birthdate || '').split('/')[2] +
      '&birthyearfilter=' +
      '&deathyear=&deathyearfilter=&location=&locationId=&memorialid=&mcid=&linkedToName=&datefilter=&orderby=r&plot=';
    },
  },
  mounted() {
    console.log('mounted');
